...
#define ID_LIT_MAXSIZE  10			// максимальное количество символов в имени литерала
...
namespace IT			// таблица идентификаторов
{
	...
	struct Entry	// строка таблицы идентификаторов
	{
		...
		char	id[ID_MAXSIZE];					// индентификатор
	...
	void writeEntry(Entry&, char* id, int idx, IDDATATYPE dt, IDTYPE t, int vint);
	void writeEntry(Entry&, char* id, int idx, IDDATATYPE dt, IDTYPE t, int len, char* str);

	void Delete(IdTable& idtable);	// удалить таблицу лексем (освободить пам€ть)
	void showTable(IdTable& idtable, std::ostream& stream);
};